use super::measures::{px, ExactLength};
use super::*;
use crate::style::s;
...
fn Row_view<Ms>(
    args: RowColumnArgs,
    mut root: Node<Ms>,
    mut children: Vec<Node<Ms>>,
    mut Item: Vec<(Node<Ms>, RowItemArgs)>,
) -> Node<Ms> {
    ...
    for item in Item.drain(0..) {
        ...
        match &item.1.flex {
            ...
            Flex::Full => item
                .0
                .style(s().flex("1 1 0%").min_width(px(0)).min_height(px(0))),
        ...
        match item.1.align {
            RowAlign::Left => {
                item.0.style(s().p(padding.clone()).name("row_item"));
                left_nodes.push(item)
            }
            RowAlign::Center => {
                item.0.style(s().p(padding.clone()).name("row_item"));
                center_nodes.push(item)
            }
            RowAlign::Right => {
                item.0.style(s().p(padding.clone()).name("row_item"));
                right_nodes.insert(0, item)
            }

            RowAlign::LeftTop => {
                item.0.style(
                    s().align_self_flex_start()
                        .p(padding.clone())
                        .name("row_item"),
                );
                left_nodes.push(item);
            ...
            RowAlign::LeftMiddle => {
                item.0
                    .style(s().align_self_center().p(padding.clone()).name("row_item"));
                left_nodes.push(item)
            }
            RowAlign::CenterMiddle => {
                item.0
                    .style(s().align_self_center().p(padding.clone()).name("row_item"));
                center_nodes.push(item)
            }
            RowAlign::RightMiddle => {
                item.0
                    .style(s().align_self_center().p(padding.clone()).name("row_item"));
                right_nodes.insert(0, item)
            }
            ...
            RowAlign::LeftBottom => {
                item.0.style(
                    s().align_self_flex_end()
                        .p(padding.clone())
                        .name("row_item"),
                );
                left_nodes.push(item)
            }
            RowAlign::CenterBottom => {
                item.0.style(
                    s().align_self_flex_end()
                        .p(padding.clone())
                        .name("row_item"),
    ...
    match (left_nodes.len(), center_nodes.len(), right_nodes.len()) {
        (0, 0, 0) => empty![],
        (0, 0, _) => root![
            s().grid_template_columns("minmax(0px, 1fr)")
                .grid_template_rows("minmax(0,1fr)"),
            div![
                s().display_flex()
                    .flex_direction_row()
                    .justify_content_flex_end(),
                right_nodes.iter().map(|(Item, _arg)| Item)
            ],
        ],
        (0, _, 0) => root![
            s().grid_template_columns("auto")
                .grid_template_rows("minmax(0,1fr)"),
            div![
                s().display_flex()
                    .flex_direction_row()
                    .justify_content_center(),
                center_nodes.iter().map(|(Item, _arg)| Item)
            ],
        ],
        (0, _, _) => root![
            s().grid_template_columns("minmax(0px, 1fr) auto minmax(0px, 1fr)")
                .grid_template_rows("minmax(0,1fr)"),
            div![],
            div![
                s().mr(gap.clone())
        ...
        (_, 0, 0) => root![
            ...
            div![
                ...
                left_nodes.iter().map(|(Item, _arg)| Item)
            ],
        ],
        (_, 0, _) => root![
            s().grid_template_columns("minmax(0px, 1fr) minmax(0px, 1fr)")
                .grid_template_rows("minmax(0,1fr)"),
            div![
                s().mr(gap.clone())
                    .display_flex()
...
#[view_macro]
#[allow(non_snake_case)]
pub fn Column_view<Ms>(
    args: RowColumnArgs,
    mut root: Node<Ms>,
    mut children: Vec<Node<Ms>>,
    mut Item: Vec<(Node<Ms>, ColumnItemArgs)>,
) -> Node<Ms> {
    let (flex, gap, padding) = (args.flex, args.gap, args.padding);
    ...
    for Item in Item.drain(0..) {
        ...
        match item.1.align {
            ColumnAlign::Top => {
                item.0.style(s().p(padding.clone()).name("col_item"));
                top_nodes.push(item)
            }
            ColumnAlign::Bottom => {
                item.0.style(s().p(padding.clone()).name("col_item"));
                bottom_nodes.insert(0, item)
            }
            ColumnAlign::Middle => {
                item.0.style(s().p(padding.clone()).name("col_item"));
                middle_nodes.push(item)
            }
            ColumnAlign::TopLeft => {
                item.0.style(
                    s().align_self_flex_start()
            ...
            ColumnAlign::TopCenter => {
                item.0
                    .style(s().align_self_center().p(padding.clone()).name("col_item"));
                top_nodes.push(item)
            }
            ColumnAlign::MiddleCenter => {
                item.0
                    .style(s().align_self_center().p(padding.clone()).name("col_item"));
                middle_nodes.push(item)
            }
            ColumnAlign::BottomCenter => {
                item.0
                    .style(s().align_self_center().p(padding.clone()).name("col_item"));
                bottom_nodes.insert(0, item)
            }
            ColumnAlign::TopRight => {
                item.0.style(
                    s().align_self_flex_end()
                        .p(padding.clone())
                        .name("col_item"),
    ...
    match (top_nodes.len(), middle_nodes.len(), bottom_nodes.len()) {
        (0, 0, 0) => empty![],
        (0, 0, _) => root![
            s().grid_template_rows("minmax(0px, 1fr)")
                .grid_template_columns("minmax(0,1fr)"),
            div![
                s().display_flex()
                    .flex_direction_column()
                    .justify_content_flex_end(),
                bottom_nodes.iter().map(|(Item, _arg)| Item)
            ],
        ],
        (0, _, 0) => root![
            ...
            div![
                s().display_flex()
                    .flex_direction_column()
                    .justify_content_center(),
                middle_nodes.iter().map(|(Item, _arg)| Item)
            ],
        ],
        (0, _, _) => root![
            s().grid_template_rows("minmax(0px, 1fr) auto minmax(0px, 1fr)")
                .grid_template_columns("minmax(0,1fr)"),
            div![],
            div![
                s().mb(gap.clone())
        ...
        (_, 0, 0) => root![
            ...
            div![
                s().display_flex()
                    .flex_direction_column()
                    .justify_content_flex_start(),
                top_nodes.iter().map(|(Item, _arg)| Item)
            ],
        ],
        (_, 0, _) => root![
            s().grid_template_rows("minmax(0px, 1fr) minmax(0px, 1fr)")
                .grid_template_columns("minmax(0,1fr)"),
            div![
                s().mb(gap.clone())
                    .display_flex()
